...
//	void stitchDoubleMT(IndexThreadReduce<Vec10>* red, MatXX &H, VecX &b, EnergyFunctional const * const EF, bool MT)
	void stitchDoubleMT(MatXX &H, VecX &b, EnergyFunctional const * const EF, bool MT)
	{
		// sum up, splitting by bock in square.
/*
		if(MT)
		{
			MatXX Hs[num_threads_1];
			VecX bs[num_threads_1];
			for(int i=0;i<num_threads_1;i++)
			{
...
//		}
        ...
        int num_threads_1 = 1;
	AccumulatorXX<8,CPARS>* accE[1];
	AccumulatorX<8>* accEB[1];
	AccumulatorXX<8,8>* accD[1];
	AccumulatorXX<CPARS,CPARS> accHcc[1];
	AccumulatorX<CPARS> accbc[1];
	int nframes[1];
