...
double PointFrameResidual::linearize(CalibHessian* HCalib)
{
	...
	float d_d_x, d_d_y;
	{
		...
		// diff d_idepth
		d_d_x = drescale * (PRE_tTll_0[0]-PRE_tTll_0[2]*u)*SCALE_IDEPTH*HCalib->fxl();   // p_Xt / p_id
		...
		d_C_x[0] = (d_C_x[0]+u)*SCALE_F;   // p_Xt / p_intermatrix
		d_C_x[1] *= SCALE_F;
		...
		d_xi_x[0] = new_idepth*HCalib->fxl();  // p_Xt / p_pose
		d_xi_x[1] = 0;
	...
	for(int idx=0;idx<patternNum;idx++)
	{
...

		{
			...
			J->JIdx[0][idx] = hitColor[1]; // p_I / p_Xt
			J->JIdx[1][idx] = hitColor[2];
			J->JabF[0][idx] = drdA*hw;     // p_I / p_ab
			J->JabF[1][idx] = hw;
